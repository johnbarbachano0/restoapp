...
export const getIsoToday = () => {
  return new Date().toISOString(navigator.language);
};
...
// transform firebaseData to array
export const transformFirebaseResponse = (data, idName) => {
  const newArray = [];
  for (const property in data) {
    const objData = data[property];
    newArray.push({ [idName]: property, ...objData });
  }
  return newArray;
};
